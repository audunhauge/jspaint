...
function setup() {
  ...
  const pointers = g("pointers"); // turned on by keys
  B = canCanvas.getBoundingClientRect(); // x,y for top left corner of canvas

  // set pointers selected
  pointerActive = () => /**  @type {HTMLInputElement} */ (pointers).checked = true;

  renderCanvas = () => renderAll(ctx);

  divTools.addEventListener("click", e => activateTool(e, ctx, divShapelist));
